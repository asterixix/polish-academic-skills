"""Shared HTTP helper for the polish-culture-archives skill scripts.
...
this skill only (no shared deps across skills).
"""
...
def request(url: str, method: str = "GET", headers: dict | None = None,
    ...
    Returns (status_code, body_bytes, response_headers) where
    response_headers is a dict with lowercase keys.
...
def request_text(url: str, method: str = "GET", headers: dict | None = None,
    ...
    """Like request(), but decodes the response body as text.

    Encoding is taken from the response's Content-Type charset when present
    (several sources here answer with iso-8859-2), otherwise falls back to
    default_encoding.
    """
    _status, body, resp_headers = request(url, method=method, headers=headers, data=data, timeout=timeout)
...
def build_query(base_url: str, params: dict) -> str:
    ...
    List values are repeated as multiple params (e.g. subtype=post&subtype=page).
    """
    pairs = []
