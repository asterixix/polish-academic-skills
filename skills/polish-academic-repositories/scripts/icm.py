...
Subcommands (ported from icm_search / icm_get_item):
  search -- full-text + faceted discovery search. Retries once, dropping all
            filters, if the filtered query returns HTTP 400/404.
  get    -- single item metadata by UUID.

Source: https://open.icm.edu.pl
"""
...
API_BASE = "https://open.icm.edu.pl/server/api"
BASE_URL = "https://open.icm.edu.pl"
...
def summarize_item(raw: str) -> dict:
    ...
    return {
        ...
        "dateIssued": _http.dc_first(m, "dc.date.issued") or None,
        "publisher": _http.dc_first(m, "dc.publisher") or None,
        "affiliation": _http.dc_first(m, "dc.contributor.affiliation") or None,
...
def _build_params(args: argparse.Namespace, use_all_filters: bool) -> list:
    ...
    if args.publisher:
        _http.add_dspace_filter(params, "publisher", args.publisher, "contains")
    if args.affiliation:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="ICM -- Open Research Data Repository (University of Warsaw) discovery search."
    )
    sub = p.add_subparsers(dest="command", required=True)

    s = sub.add_parser("search", help="Full-text + faceted discovery search over publications and research data.")
    ...
    s.add_argument("--publisher", help="Publisher filter (default op: contains).")
